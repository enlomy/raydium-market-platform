...
export class BaseMpl {
  ...
  async getUpdateMetadataIx(token: web3.PublicKey, owner: web3.PublicKey, input: any) {
    ...
    try {
      ...
      const tx = new web3.Transaction().add(metaUpdateix, updateCuIx, ...ixs);
      tx.feePayer = owner;
      tx.recentBlockhash = recentBlockhash
      // tx.sign(this.wallet.payer)
      return tx;
  ...
  async getUpdateAuthorityIx(token: web3.PublicKey, owner: web3.PublicKey, freezeRevokeAuthorities: boolean, mintRevokeAuthorities: boolean) {
